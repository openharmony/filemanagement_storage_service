 ...
 */
#include "storagedaemonprovidermountdlpfuse_fuzzer.h"
...
bool StorageDaemonProviderMountDlpFuseFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_MOUNT_DLP_FUSE);
    MessageParcel datas;
...
{
    OHOS::StorageDaemonProviderMountDlpFuseFuzzTest(data, size);
    return 0;
}