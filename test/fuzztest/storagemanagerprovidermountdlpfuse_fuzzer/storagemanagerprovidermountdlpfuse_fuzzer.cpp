 ...
 */
#include "storagemanagerprovidermountdlpfuse_fuzzer.h"
...
bool MountDlpFuseFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_MOUNT_DLP_FUSE);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::StorageManager::MountDlpFuseFuzzTest(data, size);
    return 0;
}