 ...
 */
#include "storagedaemonprovidergetdisksize_fuzzer.h"
...
bool GetDiskSizeFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_DISK_SIZE);
    MessageParcel datas;
...
{
    OHOS::GetDiskSizeFuzzTest(data, size);
    return 0;
}
