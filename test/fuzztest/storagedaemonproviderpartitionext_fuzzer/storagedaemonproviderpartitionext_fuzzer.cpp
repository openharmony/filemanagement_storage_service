 ...
 */
#include "storagedaemonproviderpartitionext_fuzzer.h"
...
bool PartitionExtFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_PARTITION_IN_STRING_IN_STRING);
    MessageParcel datas;
...
{
    OHOS::PartitionExtFuzzTest(data, size);
    return 0;
}
