 ...
 */
#include "storagedaemonproviderformatpartitionext_fuzzer.h"
...
bool FormatPartitionExtFuzzTest(const uint8_t *data, size_t size)
{
    ...
    datas.WriteBool(fdp.ConsumeBool());
    datas.RewindRead(0);
    MessageParcel reply;
...
{
    OHOS::FormatPartitionExtFuzzTest(data, size);
    return 0;
}
