 ...
 */
#include "storagemanagerprovidergetcurrentbundleinodes_fuzzer.h"
...
void GetCurrentBundleInodes(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_GET_CURRENT_BUNDLE_INODES);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    GetCurrentBundleInodes(data, size);
    return 0;
}
