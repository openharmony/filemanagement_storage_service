 ...
 */
#include "storagedaemonproviderumountdlpfuse_fuzzer.h"
...
bool StorageDaemonProviderUMountDlpFuseFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_UMOUNT_DLP_FUSE);
    MessageParcel datas;
...
{
    OHOS::StorageDaemonProviderUMountDlpFuseFuzzTest(data, size);
    return 0;
}
