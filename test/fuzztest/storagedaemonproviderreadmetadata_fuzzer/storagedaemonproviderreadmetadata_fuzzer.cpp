 ...
 */
#include "storagedaemonproviderreadmetadata_fuzzer.h"
...
bool ReadMetadataFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_READ_METADATA);
    MessageParcel datas;
...
{
    OHOS::ReadMetadataFuzzTest(data, size);
    return 0;
}
