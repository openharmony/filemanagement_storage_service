 ...
 */
#include "storagemanagerprovidernotifycreatebundledatadirwithel_fuzzer.h"
...
bool NotifyCreateBundleDataDirWithElFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_NOTIFY_CREATE_BUNDLE_DATA_DIR_WITH_EL);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::StorageManager::NotifyCreateBundleDataDirWithElFuzzTest(data, size);
    return 0;
}
