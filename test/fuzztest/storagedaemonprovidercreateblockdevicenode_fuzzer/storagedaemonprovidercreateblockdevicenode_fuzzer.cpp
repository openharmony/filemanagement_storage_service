 ...
 */
#include "storagedaemonprovidercreateblockdevicenode_fuzzer.h"
...
bool CreateBlockDeviceNodeFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_CREATE_BLOCK_DEVICE_NODE);
    MessageParcel datas;
...
{
    OHOS::CreateBlockDeviceNodeFuzzTest(data, size);
    return 0;
}
