 ...
 */
#include "storagedaemonprovidergetancosizedata_fuzzer.h"
...
bool GetAncoSizeDataFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_ANCO_SIZE_DATA);
    MessageParcel datas;
...
{
    OHOS::GetAncoSizeDataFuzzTest(data, size);
    return 0;
}