 ...
 */
#include "storagedaemonprovidergetcapacity_fuzzer.h"
...
bool GetCapacityFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_CAPACITY);
    MessageParcel datas;
...
{
    OHOS::GetCapacityFuzzTest(data, size);
    return 0;
}
