 ...
 */
#include "storagedaemonprovidererasealluserencryptedkeys_fuzzer.h"
...
bool EraseAllUserEncryptedKeysFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_ERASE_ALL_USER_ENCRYPTED_KEYS);
    MessageParcel datas;
...
{
    OHOS::EraseAllUserEncryptedKeysFuzzTest(data, size);
    return 0;
}
