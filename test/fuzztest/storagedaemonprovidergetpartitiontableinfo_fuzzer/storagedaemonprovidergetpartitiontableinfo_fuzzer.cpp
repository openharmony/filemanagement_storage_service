 ...
 */
#include "storagedaemonprovidergetpartitiontableinfo_fuzzer.h"
...
bool GetPartitionTableInfoFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_PARTITION_TABLE_INFO);
    MessageParcel datas;
...
{
    OHOS::GetPartitionTableInfoFuzzTest(data, size);
    return 0;
}
