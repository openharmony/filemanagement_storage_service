 ...
 */
#include "storagemanagerprovidergetuserstoragestatswithuserid_fuzzer.h"
...
bool GetUserStorageStatsWithUserIdFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code =
        static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_GET_USER_STORAGE_STATS_IN_INT_OUT_STORAGESTATS);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    GetUserStorageStatsWithUserIdFuzzTest(data, size);
    return 0;
}
