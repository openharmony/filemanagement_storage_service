 ...
 */
#include "storagemanagerprovidererasealluserencryptedkeys_fuzzer.h"
...
bool PrepareEraseAllUserEncryptedKeysFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_ERASE_ALL_USER_ENCRYPTED_KEYS);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    PrepareEraseAllUserEncryptedKeysFuzzTest(data, size);
    return 0;
}
