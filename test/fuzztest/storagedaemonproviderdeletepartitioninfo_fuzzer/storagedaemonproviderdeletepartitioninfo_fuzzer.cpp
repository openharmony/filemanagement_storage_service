 ...
 */
#include "storagedaemonproviderdeletepartitioninfo_fuzzer.h"
...
bool DeletePartitionInfoFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_DELETE_PARTITION_INFO);
    MessageParcel datas;
...
{
    OHOS::DeletePartitionInfoFuzzTest(data, size);
    return 0;
}
