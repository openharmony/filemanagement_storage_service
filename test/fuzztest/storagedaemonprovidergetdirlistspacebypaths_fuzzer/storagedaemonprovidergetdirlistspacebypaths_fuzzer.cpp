 ...
 */
#include "storagedaemonprovidergetdirlistspacebypaths_fuzzer.h"
...
void GetDirListSpaceByPathsFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size < sizeof(int32_t))) {
        return;
    }
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_DIR_LIST_SPACE_BY_PATHS);
    MessageParcel datas;
...
{
    OHOS::GetDirListSpaceByPathsFuzzTest(data, size);
    return 0;
}