 ...
 */
#include "storagedaemonprovidergetsystemdatasize_fuzzer.h"
...
void GetSystemDataSizeFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size < sizeof(int32_t))) {
        return;
    }
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_SYSTEM_DATA_SIZE);
    MessageParcel datas;
...
{
    OHOS::GetSystemDataSizeFuzzTest(data, size);
    return 0;
}