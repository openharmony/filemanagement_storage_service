 ...
 */
#include "storagedaemonproviderexecutecommand_fuzzer.h"
...
bool ExecuteCommandFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_EXECUTE_COMMAND);
    MessageParcel datas;
    datas.WriteInterfaceToken(StorageDaemonStub::GetDescriptor());
    std::vector<std::string> cmd;
    cmd.push_back(fdp.ConsumeRandomLengthString(MAX_PATH_LENGTH));
    cmd.push_back(fdp.ConsumeRandomLengthString(MAX_PATH_LENGTH));
    cmd.push_back(fdp.ConsumeRandomLengthString(MAX_PATH_LENGTH));
    datas.WriteStringVector(cmd);
    datas.RewindRead(0);
    MessageParcel reply;
...
{
    OHOS::ExecuteCommandFuzzTest(data, size);
    return 0;
}
