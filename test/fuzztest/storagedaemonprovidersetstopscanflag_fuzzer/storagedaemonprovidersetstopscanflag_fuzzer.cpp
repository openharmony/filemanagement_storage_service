 ...
 */
#include "storagedaemonprovidersetstopscanflag_fuzzer.h"
...
bool SetStopScanFlagFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_SET_STOP_SCAN_FLAG);
    MessageParcel datas;
...
{
    OHOS::SetStopScanFlagFuzzTest(data, size);
    return 0;
}