 ...
 */
#include "storagemanagerproviderumountdlpfuse_fuzzer.h"
...
bool UMountDlpFuseFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageManagerIpcCode::COMMAND_UMOUNT_DLP_FUSE);
    MessageParcel datas;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::StorageManager::UMountDlpFuseFuzzTest(data, size);
    return 0;
}