 ...
 */
#include "storagedaemonprovidergetdirlistspace_fuzzer.h"
...
bool GetDirListSpaceFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_DIR_LIST_SPACE);
    MessageParcel datas;
...
{
    OHOS::GetDirListSpaceFuzzTest(data, size);
    return 0;
}