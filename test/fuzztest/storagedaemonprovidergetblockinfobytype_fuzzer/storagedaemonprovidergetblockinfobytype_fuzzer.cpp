 ...
 */
#include "storagedaemonprovidergetblockinfobytype_fuzzer.h"
...
bool GetBlockInfoByTypeFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_GET_BLOCK_INFO_BY_TYPE);
    MessageParcel datas;
...
{
    OHOS::GetBlockInfoByTypeFuzzTest(data, size);
    return 0;
}
