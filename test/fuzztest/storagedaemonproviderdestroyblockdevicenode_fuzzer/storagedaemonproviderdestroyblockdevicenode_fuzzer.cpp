 ...
 */
#include "storagedaemonproviderdestroyblockdevicenode_fuzzer.h"
...
bool DestroyBlockDeviceNodeFuzzTest(const uint8_t *data, size_t size)
{
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_DESTROY_BLOCK_DEVICE_NODE);
    MessageParcel datas;
...
{
    OHOS::DestroyBlockDeviceNodeFuzzTest(data, size);
    return 0;
}
