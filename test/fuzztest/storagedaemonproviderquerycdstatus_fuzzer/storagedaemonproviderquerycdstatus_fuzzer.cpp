 ...
 */
#include "storagedaemonproviderquerycdstatus_fuzzer.h"
...
bool QueryCDStatusFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_QUERY_C_D_STATUS);
    MessageParcel datas;
...
{
    OHOS::QueryCDStatusFuzzTest(data, size);
    return 0;
}
