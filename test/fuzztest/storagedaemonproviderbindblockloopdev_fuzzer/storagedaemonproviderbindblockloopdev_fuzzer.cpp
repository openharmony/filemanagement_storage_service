 ...
 */
#include "storagedaemonproviderbindblockloopdev_fuzzer.h"
...
bool BindBlockLoopDevFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size == 0)) {
        return false;
    }
    FuzzedDataProvider fdp(data, size);
    uint32_t code = static_cast<uint32_t>(IStorageDaemonIpcCode::COMMAND_BIND_BLOCK_LOOP_DEV);
    MessageParcel datas;
    datas.WriteInterfaceToken(StorageDaemonStub::GetDescriptor());
    datas.WriteString(fdp.ConsumeRandomLengthString(MAX_PATH_LENGTH));
    datas.WriteUint64(fdp.ConsumeIntegral<uint64_t>());
    datas.WriteUint64(fdp.ConsumeIntegral<uint64_t>());
    datas.RewindRead(0);
    MessageParcel reply;
...
{
    OHOS::BindBlockLoopDevFuzzTest(data, size);
    return 0;
}
