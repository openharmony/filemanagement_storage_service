...
- 高危删除约束：对路径执行不可逆删除（递归删除、`RmDirRecurse`/`ForceRemoveDirectory`、`unlink`、`rmdir` 等）前，必须确认目标真实后端即预期对象——挂载类间接层（设备挂载/绑定挂载/重复或多层挂载等）会使挂载点目录的真实后端≠表面，对残留挂载层的挂载点递归删会穿透到源数据导致数据丢失。删挂载点目录前必须逐层彻底卸载、确认无残留挂载层，否则禁止执行、需人工确认。
