                                                                       ...
                                                                       std::vector<T> &parcelableInfos)
{
    ...
    if (dataSize > MAX_IPC_REWDATA_SIZE) {
        ...
        if (GetParcelInfoFromAshMem(reply, buffer, ashMemSize) != ERR_OK) {
            LOGE("read data from ashmem fail, length %{public}zu", dataSize);
            return ERR_APPEXECFWK_PARCEL_ERROR;
        }
        if (dataSize > static_cast<size_t>(ashMemSize)) {
            LOGE("dataSize %{public}zu exceeds ashmem size %{public}d", dataSize, ashMemSize);
            free(buffer);
            return ERR_APPEXECFWK_PARCEL_ERROR;
        }
    } else {
...
ErrCode BundleManagerAdapterProxy::GetParcelInfoFromAshMem(MessageParcel &reply, void *&data, int32_t &ashMemSize)
{
    ashMemSize = 0;
    ...
    ashMemSize = ashMem->GetAshmemSize();
...
ErrCode BundleManagerAdapterProxy::GetApplicationInfosV9(int32_t flags, int32_t userId,
    std::vector<ApplicationInfo> &appInfos)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        LOGE("fail to GetApplicationInfosV9 due to write InterfaceToken fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteInt32(flags)) {
        LOGE("fail to GetApplicationInfosV9 due to write flags fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteInt32(userId)) {
        LOGE("fail to GetApplicationInfosV9 due to write userId fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
...
ErrCode BundleManagerAdapterProxy::CleanBundlePartialCacheAutomatic(const CleanCacheInfo &cacheInfo,
    uint64_t &beforeCleanedSize, uint64_t &afterCleanedSize)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        LOGE("fail to CleanBundlePartialCacheAutomatic due to write InterfaceToken fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteParcelable(&cacheInfo)) {
        LOGE("fail to CleanBundlePartialCacheAutomatic due to write bundleName fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }

    MessageParcel reply;
    if (!SendTransactCmdWithLog(BundleMgrInterfaceCode::AUTO_CLEAN_PARTIAL_CACHE, data, reply)) {
        LOGE("fail to CleanBundlePartialCacheAutomatic from server");
        return ERR_BUNDLE_MANAGER_IPC_TRANSACTION;
    }
    ErrCode result = reply.ReadInt32();
    if (result != ERR_OK) {
        LOGE("CleanBundlePartialCacheAutomatic failed with ret=%{public}d", result);
        return result;
    }
    if (!reply.ReadUint64(beforeCleanedSize)) {
        LOGE("fail to CleanBundlePartialCacheAutomatic due to read beforeCleanedSize fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!reply.ReadUint64(afterCleanedSize)) {
        LOGE("fail to CleanBundlePartialCacheAutomatic due to read afterCleanedSize fail");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    LOGD("CleanBundlePartialCacheAutomatic success, bundleName=%{public}s, userId=%{public}d, "
         "appIndex=%{public}d, beforeCleanedSize=%{public}llu, afterCleanedSize=%{public}llu",
         cacheInfo.bundleName.c_str(), cacheInfo.userId, cacheInfo.appIndex,
         static_cast<unsigned long long>(beforeCleanedSize),
         static_cast<unsigned long long>(afterCleanedSize));
    return ERR_OK;
}
} // namespace StorageSpaceManager
} // namespace OHOS
