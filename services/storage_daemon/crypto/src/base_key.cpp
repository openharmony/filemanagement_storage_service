...
{
    LOGI("[L4:BaseKey] InitKey: >>> ENTER <<< dir=%{public}s, needGenerateKey=%{public}d",
        dir_.c_str(), needGenerateKey);
...
{
    LOGI("[L4:BaseKey] ClearKeyInfo: >>> ENTER <<< dir=%{public}s", dir_.c_str());
    keyInfo_.key.Clear();
}
...
int BaseKey::GetCandidateVersion() const
{
    ...
    if (candidate != -1) {
        LOGW("[L4:BaseKey] GetCandidateVersion: candidate key version is %{public}d", candidate);
    }
    return candidate;
}
...
#endif
{
    LOGI("[L4:BaseKey] StoreKey: >>> ENTER <<< dir=%{public}s", dir_.c_str());
    ...
    if (ret == E_OK) {
        ...
        LOGI("[L4:BaseKey] StoreKey: rename %{public}s to %{public}s", pathTemp.c_str(), candidate.c_str());
        if (rename(pathTemp.c_str(), candidate.c_str()) == 0) {
            LOGI("[L4:BaseKey] StoreKey: start sync");
            SyncKeyDir();
            LOGI("[L4:BaseKey] StoreKey: <<< EXIT SUCCESS <<< dir=%{public}s", dir_.c_str());
            return E_OK;
        }
        LOGE("[L4:BaseKey] StoreKey: <<< EXIT FAILED <<< dir=%{public}s, rename failed, errno=%{public}d",
            dir_.c_str(), errno);
        ret = errno;
    } else {
        StorageService::StorageRadar::ReportUserKeyResult("StoreKey", 0, ret, "", "dir_=" + dir_);
        LOGE("[L4:BaseKey] StoreKey: <<< EXIT FAILED <<< dir=%{public}s, DoStoreKey failed, ret=%{public}d",
            dir_.c_str(), ret);
    }
    OHOS::ForceRemoveDirectory(pathTemp);
    LOGI("[L4:BaseKey] StoreKey: start sync");
    SyncKeyDir();
    LOGI("[L4:BaseKey] StoreKey: sync end");
    return ret;
}
...
#endif
{
    LOGI("[L4:BaseKey] DoStoreKey: >>> ENTER <<< dir=%{public}s", dir_.c_str());
    auto pathTemp = dir_ + PATH_KEY_TEMP;
    ...
    if (keyType == TYPE_EL1 || keyType == TYPE_GLOBAL_EL1) {
        auto ret = EncryptDe(auth, pathTemp);
        LOGI("[L4:BaseKey] DoStoreKey: <<< EXIT %s <<< dir=%{public}s",
            ret == E_OK ? "SUCCESS" : "FAILED", dir_.c_str());
        return ret;
    }
    if ((auth.token.IsEmpty() && auth.secret.IsEmpty()) || // Create user/Delete pincode(EL2-4)
        !auth.token.IsEmpty()) {  // add/change pin code (EL2-4)
        LOGE("[L4:BaseKey] DoStoreKey: Encrypt with HUKS and OpenSSL");
        KeyContext keyCtx = {};
    ...
    LOGI("[L4:BaseKey] DoStoreKey: <<< EXIT SUCCESS <<< dir=%{public}s", dir_.c_str());
    return E_OK;
}
...
bool BaseKey::CheckAndUpdateVersion()
{
    ...
    if (OHOS::LoadStringFromFile(pathVersion, version)) {
        ...
        LOGE("[L4:BaseKey] CheckAndUpdateVersion: save version failed, errno:%{public}d", errno);
        return false;
    }
...
{
    LOGI("[L4:BaseKey] InitKeyContext: >>> ENTER <<< keyPath=%{public}s", keyPath.c_str());
    auto ret = LoadAndSaveShield(auth, keyPath + PATH_SHIELD, true, keyCtx);
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] InitKeyContext: <<< EXIT FAILED <<< LoadAndSaveShield failed");
        return ret;
    }

    if (!GenerateAndSaveKeyBlob(keyCtx.secDiscard, keyPath + PATH_SECDISC, CRYPTO_KEY_SECDISC_SIZE)) {
        LOGE("[L4:BaseKey] InitKeyContext: <<< EXIT FAILED <<< Generate sec_discard failed");
        return E_GENERATE_DISCARD_ERROR;
    }

    if (!GenerateKeyBlob(keyCtx.nonce, GCM_NONCE_BYTES) ||
        !GenerateKeyBlob(keyCtx.aad, GCM_MAC_BYTES)) {
        LOGE("[L4:BaseKey] InitKeyContext: <<< EXIT FAILED <<< Generate nonce and aad failed");
        return E_KEY_BLOB_ERROR;
    }
    LOGI("[L4:BaseKey] InitKeyContext: <<< EXIT SUCCESS <<<");
    return E_OK;
}
                                   ...
                                   bool needGenerateShield, KeyContext &keyCtx)
{
    ...
    if (needGenerateShield) {
        ...
        if (ret != HKS_SUCCESS) {
            LOGE("[L4:BaseKey] LoadAndSaveShield: <<< EXIT FAILED <<< GenerateKey of shield failed");
            return ret;
        }
    } else {
...
{
    LOGI("[L4:BaseKey] UpdateKey: >>> ENTER <<< keypath=%{public}s, needSyncCandidate=%{public}d",
        keypath.c_str(), needSyncCandidate);
    if (!needSyncCandidate) {
        LOGE("[L4:BaseKey] UpdateKey: skip candidate update");
        return E_OK;
    }
    auto candidate = keypath.empty() ? GetCandidateDir() : keypath;
    LOGI("[L4:BaseKey] UpdateKey: ready to update, candidate=%{public}s", candidate.c_str());
    if (candidate.empty() && GetTypeFromDir() == TYPE_EL5) {
        LOGI("[L4:BaseKey] UpdateKey: no uece candidate dir, do not need updateKey");
        return E_OK;
    }
    if (candidate.empty()) {
        LOGE("[L4:BaseKey] UpdateKey: <<< EXIT FAILED <<< candidate dir is empty");
    ...
    if (strcmp(candidate.c_str(), (dir_ + PATH_LATEST).c_str()) != 0) {
        ...
        if (ret != E_OK) {
            LOGE("[L4:BaseKey] UpdateKey: <<< EXIT FAILED <<< UpdateOrRollbackKey failed, errno=%{public}d", errno);
            return ret;
        }
    }
    ...
    SyncKeyDir();
    LOGI("[L4:BaseKey] UpdateKey: <<< EXIT SUCCESS <<< dir=%{public}s", dir_.c_str());
    return E_OK;
}
...
int32_t BaseKey::UpdateOrRollbackKey(const std::string &candidate)
{
    ...
    if (IsDir(latestPath)) { // rename latest to latest_backup
        ...
        LOGI("[L4:BaseKey] UpdateOrRollbackKey: restore the latest_backup success");
    }
...
{
    LOGI("[L4:BaseKey] EncryptDe: >>> ENTER <<< path=%{public}s", path.c_str());
    KeyContext ctxDe;
    auto ret = InitKeyContext(auth, path, ctxDe);
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] EncryptDe: <<< EXIT FAILED <<< InitKeyContext failed");
        return ret;
    }
    keyEncryptType_ = KeyEncryptType::KEY_CRYPT_HUKS;
    ...
    if (!SaveKeyBlob(ctxDe.rndEnc, path + PATH_ENCRYPTED)) {
        LOGE("[L4:BaseKey] EncryptDe: <<< EXIT FAILED <<< SaveKeyBlob rndEnc failed");
        ClearKeyContext(ctxDe);
        return E_SAVE_KEY_BLOB_ERROR;
    }
...
{
    LOGI("[L4:BaseKey] EncryptEceSece: >>> ENTER <<< keyType=%{public}u", keyType);
    // rnd 64 -> rndEnc 80
    ...
    LOGI("[L4:BaseKey] EncryptEceSece: Encrypt by openssl start"); // rndEnc 80 -> rndEncEnc 108
    ret = OpensslCrypto::AESEncrypt(mUserAuth.secret, rndEnc, keyCtx);
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] EncryptEceSece: <<< EXIT FAILED <<< Encrypt by openssl failed");
        return ret;
    }
    LOGE("[L4:BaseKey] EncryptEceSece: Encrypt by openssl end");
    rndEnc.Clear();
    keyEncryptType_ = KeyEncryptType::KEY_CRYPT_HUKS_OPENSSL;
    LOGI("[L4:BaseKey] EncryptEceSece: <<< EXIT SUCCESS <<<");
    return E_OK;
}
...
int32_t BaseKey::RestoreKey(const UserAuth &auth, bool needSyncCandidate, bool needFixFiles)
{
    LOGD("[L4:BaseKey] RestoreKey: >>> ENTER <<< auth token %{public}d, auth secret %{public}d,"
        " needFixFiles=%{public}d", auth.token.IsEmpty(), auth.secret.IsEmpty(), needFixFiles);
    ...
    if (ret == E_OK) {
        ...
        LOGI("[L4:BaseKey] RestoreKey: <<< EXIT SUCCESS <<< dir=%{public}s", dir_.c_str());
        return E_OK;
    }

    LOGE("[L4:BaseKey] RestoreKey: DoRestoreKey with %{public}s failed, ret=%{public}d", candidate.c_str(), ret);
    ...
    LOGE("[L4:BaseKey] RestoreKey: <<< EXIT FAILED <<< all restore attempts failed, ret=%{public}d", ret);
    return ret;
}

int32_t BaseKey::RestoreKey4Nato(const std::string &keyDir, KeyType type)
{
    LOGI("[L4:BaseKey] RestoreKey4Nato: >>> ENTER <<< keyDir=%{public}s, type=%{public}u", keyDir.c_str(), type);
    ...
    if (iter == keyTypeMap.end()) {
        LOGE("[L4:BaseKey] RestoreKey4Nato: <<< EXIT FAILED <<< invalid type=%{public}u", type);
        return E_PARAMS_INVALID;
    }
    UserAuth auth = { {}, {} };
    auto ret = DoRestoreKeyCeEceSece(auth, keyDir + PATH_LATEST, iter->second);
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] RestoreKey4Nato: <<< EXIT FAILED <<< DoRestoreKeyCeEceSece failed");
        return ret;
    }
    ret = StoreKey({ {}, {}, 0 });
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] RestoreKey4Nato: <<< EXIT FAILED <<< StoreKey failed");
        return ret;
    }
    ret = UpdateKey();
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] RestoreKey4Nato: <<< EXIT FAILED <<< UpdateKey failed");
        return ret;
    }
    LOGI("[L4:BaseKey] RestoreKey4Nato: <<< EXIT SUCCESS <<< keyDir=%{public}s, type=%{public}u", keyDir.c_str(), type);
    return E_OK;
}

int32_t BaseKey::DoRestoreKeyOld(const UserAuth &auth, const std::string &path)
{
    LOGW("[L4:BaseKey] DoRestoreKeyOld: >>> ENTER <<< path=%{public}s", path.c_str());
    ...
    if (!auth.secret.IsEmpty() && FileExists(NEED_UPDATE_PATH)) {
        ...
        LOGI("[L4:BaseKey] DoRestoreKeyOld: set keyEncryptType_ as KEY_CRYPT_OPENSSL success");
    } else {
        keyEncryptType_ = KeyEncryptType::KEY_CRYPT_HUKS;
        LOGI("[L4:BaseKey] DoRestoreKeyOld: set keyEncryptType_ as KEY_CRYPT_HUKS success");
    }
    auto ver = KeyCtrlLoadVersion(dir_.c_str());
    if (ver == FSCRYPT_INVALID || ver != keyInfo_.version) {
        LOGE("[L4:BaseKey] DoRestoreKeyOld: <<< EXIT FAILED <<< bad version loaded %{public}u not expected %{public}u",
            ver, keyInfo_.version);
        return E_PARAMS_INVALID;
    }
    if (!LoadKeyBlob(keyContext_.rndEnc, path + PATH_ENCRYPTED)) {
        LOGE("[L4:BaseKey] DoRestoreKeyOld: <<< EXIT FAILED <<< Load rndEnc failed");
        return E_LOAD_KEY_BLOB_ERROR;
    }
    if (keyEncryptType_ == KeyEncryptType::KEY_CRYPT_HUKS) {
        if (!LoadKeyBlob(keyContext_.shield, path + PATH_SHIELD)) {
            ...
            LOGE("[L4:BaseKey] DoRestoreKeyOld: <<< EXIT FAILED <<< Load shield failed");
            return E_LOAD_KEY_BLOB_ERROR;
        }
    }
    if (!LoadKeyBlob(keyContext_.secDiscard, path + PATH_SECDISC, CRYPTO_KEY_SECDISC_SIZE)) {
        ...
        LOGE("[L4:BaseKey] DoRestoreKeyOld: <<< EXIT FAILED <<< Load secDiscard failed");
        return E_LOAD_KEY_BLOB_ERROR;
    }
    auto ret = Decrypt(auth);
    LOGD("[L4:BaseKey] DoRestoreKeyOld: <<< EXIT %s <<<", ret == E_OK ? "SUCCESS" : "FAILED");
    return ret;
}

int32_t BaseKey::DoRestoreKeyDe(const UserAuth &auth, const std::string &path)
{
    LOGI("[L4:BaseKey] DoRestoreKeyDe: >>> ENTER <<< path=%{public}s", path.c_str());
    KeyContext ctxNone;
    if (!LoadKeyBlob(ctxNone.rndEnc, path + PATH_ENCRYPTED)) {
        LOGE("[L4:BaseKey] DoRestoreKeyDe: <<< EXIT FAILED <<< Load rndEnc failed");
        return E_LOAD_KEY_BLOB_ERROR;
    }
    ...
    LOGW("[L4:BaseKey] DoRestoreKeyDe: Decrypt by hks start");  // keyCtx.rndEnc 80 -> 64
    ...
    ClearKeyContext(ctxNone);
    LOGI("[L4:BaseKey] DoRestoreKeyDe: <<< EXIT SUCCESS <<<");
    return E_OK;
}
...
int32_t BaseKey::DoRestoreKeyCeEceSece(const UserAuth &auth, const std::string &path, const uint32_t keyType)
{
    ...
    if ((auth.secret.IsEmpty() && auth.token.IsEmpty()) ||
        ...
        KeyContext ctxNone;
        if (!LoadKeyBlob(ctxNone.rndEnc, path + PATH_ENCRYPTED)) {
            LOGE("[L4:BaseKey] DoRestoreKeyCeEceSece: <<< EXIT FAILED <<< Load rndEnc failed");
            return E_LOAD_KEY_BLOB_ERROR;
        }
        ...
        if (!SplitKeyCtx(ctxNone.rndEnc, ctxNone.nonce, ctxNone.rndEnc, ctxNone.aad)) {
            ctxNone.rndEnc.Clear();
            LOGE("[L4:BaseKey] DoRestoreKeyCeEceSece: <<< EXIT FAILED <<< Split key context failed");
            return E_SPILT_KEY_CTX_ERROR;
        }
        if (!LoadKeyBlob(ctxNone.secDiscard, path + PATH_SECDISC) ||
            !LoadKeyBlob(ctxNone.shield, path + PATH_SHIELD)) {
            ctxNone.rndEnc.Clear();
            LOGE("[L4:BaseKey] DoRestoreKeyCeEceSece: <<< EXIT FAILED <<< Load shield failed");
    ...
    if (auth.secret.IsEmpty() && !auth.token.IsEmpty()) {
        if (!keyContext_.shield.IsEmpty() || !keyContext_.rndEnc.IsEmpty()) {
            LOGI("[L4:BaseKey] DoRestoreKeyCeEceSece: Restore key by face/finger");
            UserAuth mUserAuth = auth;
            mUserAuth.secret = KeyBlob(NULL_SECRET);
            KeyContext tempCtx = {};
...
int32_t BaseKey::DoRestoreKey(const UserAuth &auth, const std::string &path)
{
    auto ver = KeyCtrlLoadVersion(dir_.c_str());
    if (ver == FSCRYPT_INVALID || ver != keyInfo_.version) {
        LOGE("[L4:BaseKey] DoRestoreKey: <<< EXIT FAILED <<< bad version loaded %{public}u not expected %{public}u",
            ver, keyInfo_.version);
        return E_VERSION_ERROR;
    }
    ...
    if (std::filesystem::exists(path + SUFFIX_NEED_RESTORE, errCode)) {
        if (restore_version < RESTORE_VERSION) {
            LOGW("[L4:BaseKey] DoRestoreKey: Old DOUBLE_2_SINGLE");
            ret = DoUpdateRestore(auth, path);
        } else {
            LOGW("[L4:BaseKey] DoRestoreKey: New DOUBLE_2_SINGLE");
...
{
    LOGI("[L4:BaseKey] DoUpdateRestore: >>> ENTER <<< keyPath=%{public}s", keyPath.c_str());
    auto ret = DoRestoreKeyOld(auth, keyPath);
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] DoUpdateRestore: <<< EXIT FAILED <<< Restore old failed");
        return ret;
    }
    std::error_code errCode;
    if (std::filesystem::exists(dir_ + PATH_NEED_RESTORE_SUFFIX, errCode) && !auth.token.IsEmpty()) {
        LOGE("[L4:BaseKey] DoUpdateRestore: Double 2 single, skip huks -> huks-openssl");
        return E_OK;
    }
    ...
    if ((userId < StorageService::START_APP_CLONE_USER_ID || userId >= StorageService::MAX_APP_CLONE_USER_ID) &&
        ...
        LOGE("[L4:BaseKey] DoUpdateRestore: <<< EXIT FAILED <<< Get secure uid from iam failed");
    ...
    ret = UpdateKey();
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] DoUpdateRestore: <<< EXIT FAILED <<< Update old failed");
        return ret;
    }
...
int32_t BaseKey::DoUpdateRestoreVx(const UserAuth &auth, const std::string &keyPath, UpdateVersion update_version)
{
    LOGI("[L4:BaseKey] DoUpdateRestoreVx: >>> ENTER <<< keyPath=%{public}s, version=%{public}u",
        keyPath.c_str(), update_version);
    auto ret = DoRestoreKeyCeEceSece(auth, keyPath, GetTypeFromDir());
    if (ret != E_OK) {
        LOGE("[L4:BaseKey] DoUpdateRestoreVx: <<< EXIT FAILED <<< Restore ce ece sece failed");
        return ret;
    }
    LOGI("[L4:BaseKey] DoUpdateRestoreVx: <<< EXIT SUCCESS <<<");
    return E_OK;
}

int32_t BaseKey::DecryptReal(const UserAuth &auth, const uint32_t keyType, KeyContext &keyCtx)
{
    LOGI("[L4:BaseKey] DecryptReal: >>> ENTER <<< keyType=%{public}u", keyType);
    UserAuth mUserAuth = auth;
    if (auth.secret.IsEmpty()) {
        mUserAuth.secret = KeyBlob(NULL_SECRET);
    }
    ...
    if (ret != E_OK) { // rndEncEnc -> rndEnc
        LOGE("[L4:BaseKey] DecryptReal: <<< EXIT FAILED <<< Decrypt by openssl failed");
        return ret;
    }
    ...
    rndEnc.Clear();
    LOGI("[L4:BaseKey] DecryptReal: <<< EXIT SUCCESS <<<");
    return E_OK;
}
...
int32_t BaseKey::Decrypt(const UserAuth &auth)
{
    ...
    switch (keyEncryptType_) {
        case KeyEncryptType::KEY_CRYPT_OPENSSL:
            LOGI("[L4:BaseKey] Decrypt: Enhanced decrypt key start");
    ...
    LOGD("[L4:BaseKey] Decrypt: <<< EXIT %s <<<", ret == E_OK ? "SUCCESS" : "FAILED");
    return ret;
}

bool BaseKey::ClearKey(const std::string &mnt)
{
    LOGI("[L4:BaseKey] ClearKey: >>> ENTER <<< dir=%{public}s", dir_.c_str());
    ...
    if (std::filesystem::exists(elNeedRestorePath, errCode)) {
        needClearFlag = false;
        LOGI("[L4:BaseKey] ClearKey: needRestore flag exist, do not remove secret");
    }
#endif
    if (needClearFlag) {
        LOGI("[L4:BaseKey] ClearKey: do clear key");
        if (!IsDir(dir_)) {
            LOGE("[L4:BaseKey] ClearKey: dir not exist, do not need to remove dir");
            return (ret == E_OK);
        }
        ...
        LOGI("[L4:BaseKey] ClearKey: force remove backupDir, %{public}s", backupDir.c_str());
        OHOS::ForceRemoveDirectory(backupDir);
        LOGI("[L4:BaseKey] ClearKey: force remove dir_, %{public}s", dir_.c_str());
    ...
    LOGI("[L4:BaseKey] ClearKey: <<< EXIT SUCCESS <<< dir=%{public}s", dir_.c_str());
    return (ret == E_OK);
}
...
void BaseKey::WipingActionDir(std::string &path)
{
    ...
    LOGI("[L4:BaseKey] WipingActionDir: >>> ENTER <<< path=%{public}s", path.c_str());
    OpenSubFile(path.c_str(), fileList);
    for (const auto &it: fileList) {
        FILE *f = fopen(it.c_str(), "w");
        if (f == nullptr) {
            LOGE("[L4:BaseKey] WipingActionDir: open %{public}s failed, errno %{public}u", it.c_str(), errno);
            return;
        }
        int fd = fileno(f);
        if (fd < 0) {
        ...
        if (ret != 0) {
            LOGE("[L4:BaseKey] WipingActionDir: F2FS_IOC_SET_PIN_FILE ioctl is %{public}u, errno = %{public}u",
                ret, errno);
        }
        ...
        if (ret != 0 && errno == EOPNOTSUPP) {
            ...
            ret = ioctl(fd, F2FS_IOC_SEC_TRIM_FILE, &trimRange);
            if (ret != 0) {
                LOGE("[L4:BaseKey] WipingActionDir: F2FS_IOC_SEC_TRIM_FILE ioctl is %{public}u, errno = %{public}u",
                    ret, errno);
            }
        }
        set = 0;
        ...
        if (ret != 0) {
            LOGE("[L4:BaseKey] WipingActionDir: F2FS_IOC_SET_PIN_FILE ioctl is %{public}u", ret);
        }
        LOGI("[L4:BaseKey] WipingActionDir: WipingActionDir success");
        (void)fclose(f);
    }
}
...
void BaseKey::SyncKeyDir() const
{
    ...
    if (dir == nullptr) {
        LOGE("[L4:BaseKey] SyncKeyDir: open %{public}s failed, errno %{public}u", dir_.c_str(), errno);
        return;
    }
    int fd = dirfd(dir);
    if (fd < 0) {
        LOGE("[L4:BaseKey] SyncKeyDir: open %{public}s failed, errno %{public}d", dir_.c_str(), errno);
        sync();
        (void)closedir(dir);
        return;
    }
    LOGW("[L4:BaseKey] SyncKeyDir: start fsync, dir_ is %{public}s", dir_.c_str());
    auto startTime = StorageService::StorageRadar::RecordCurrentTime();
    if (fsync(fd) != 0) {
        LOGE("[L4:BaseKey] SyncKeyDir: fsync %{public}s failed, errno %{public}d", dir_.c_str(), errno);
        syncfs(fd);
    }
...
bool BaseKey::UpgradeKeys()
{
    ...
    for (const auto &it : versions) {
        ...
        LOGI("[L4:BaseKey] UpgradeKeys: Upgrade of %{public}s", shieldPath.c_str());
        LoadKeyBlob(keyContext_.shield, shieldPath);
        if (HuksMaster::GetInstance().UpgradeKey(keyContext_)) {
            LOGI("[L4:BaseKey] UpgradeKeys: success upgrade of %{public}s", shieldPath.c_str());
...
bool BaseKey::GetOriginKey(KeyBlob &originKey)
{
    ...
    LOGI("[L4:BaseKey] GetOriginKey: <<< EXIT SUCCESS <<<");
    return true;
}
...
bool BaseKey::GetHashKey(KeyBlob &hashKey)
{
    ...
    LOGI("[L4:BaseKey] GetHashKey: <<< EXIT SUCCESS <<<");
    return true;
}
...
bool BaseKey::GenerateHashKey()
{
    ...
    LOGI("[L4:BaseKey] GenerateHashKey: <<< EXIT SUCCESS <<<");
    return true;
}
...
{
    LOGI("[L4:BaseKey] EncryptKeyBlob: >>> ENTER <<< keyPath=%{public}s", keyPath.c_str());
    KeyContext keyCtx;
    if (!MkDirRecurse(keyPath, S_IRWXU)) {
        LOGE("[L4:BaseKey] EncryptKeyBlob: <<< EXIT FAILED <<< MkDirRecurse failed");
        return E_MKDIR_ERROR;
    }
    ...
    if (!GenerateAndSaveKeyBlob(keyCtx.secDiscard, keyPath + PATH_SECDISC, CRYPTO_KEY_SECDISC_SIZE)) {
        keyCtx.shield.Clear();
        LOGE("[L4:BaseKey] EncryptKeyBlob: <<< EXIT FAILED <<< GenerateAndSaveKeyBlob sec_discard failed");
        return E_SAVE_KEY_BLOB_ERROR;
    }
    ...
    ClearKeyContext(keyCtx);
    LOGI("[L4:BaseKey] EncryptKeyBlob: <<< EXIT SUCCESS <<<");
    return E_OK;
}
                                ...
                                KeyBlob &decryptedKey)
{
    ...
    if (!LoadKeyBlob(keyCtx.secDiscard, path + PATH_SECDISC, CRYPTO_KEY_SECDISC_SIZE)) {
        LOGE("[L4:BaseKey] DecryptKeyBlob: <<< EXIT FAILED <<< Load key secDiscard failed");
        keyCtx.shield.Clear();
        return E_LOAD_KEY_BLOB_ERROR;
    }
...
{
    LOGD("[L4:BaseKey] ClearKeyContext: >>> ENTER <<<");
    keyCtx.aad.Clear();
    keyCtx.nonce.Clear();
    keyCtx.shield.Clear();
    keyCtx.rndEnc.Clear();
...
uint32_t BaseKey::GetTypeFromDir()
{
    ...
    if (slashIndex == std::string::npos) {
        LOGE("[L4:BaseKey] GetTypeFromDir: bad dir %{public}s", dir_.c_str());
        return type;
    }
...
std::string BaseKey::GetKeyDir()
{
    ...
    switch (type) {
        ...
        default:
            ...
            LOGE("[L4:BaseKey] GetKeyDir: type is error");
            return "";
    }
}
...
uint32_t BaseKey::GetIdFromDir()
{
    ...
    LOGI("[L4:BaseKey] GetIdFromDir: dir_: %{public}s, get userId is %{public}d", dir_.c_str(), userId);
    return static_cast<uint32_t>(userId);
}

bool BaseKey::KeyDescIsEmpty()
{
    LOGI("[L4:BaseKey] KeyDescIsEmpty: The keyBlob is null? %{public}d", keyInfo_.keyDesc.IsEmpty());
