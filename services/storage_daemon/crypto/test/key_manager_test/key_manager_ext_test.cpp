...
HWTEST_F(KeyManagerExtTest, KeyManagerExt_GenerateUserKeys_001, TestSize.Level1)
{
    ...
    auto ret = KeyManagerExt::GetInstance()->GenerateUserKeys(userId, flags);
    EXPECT_EQ(ret, 0);

    flags = 0;
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(false));
    ret = KeyManagerExt::GetInstance()->GenerateUserKeys(userId, flags);
    EXPECT_EQ(ret, 0);
...
HWTEST_F(KeyManagerExtTest, KeyManagerExt_DeleteUserKeys_001, TestSize.Level1)
{
    ...
    uint32_t userId = 124;
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(false));
    EXPECT_EQ(KeyManagerExt::GetInstance()->DeleteUserKeys(userId), 0);

    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(true));
    EXPECT_CALL(*userkeyExtMocMock_, DeleteUserKey(_)).WillOnce(Return(true));
    EXPECT_EQ(KeyManagerExt::GetInstance()->DeleteUserKeys(userId), 0);
...
HWTEST_F(KeyManagerExtTest, KeyManagerExt_ActiveUserKey_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(KeyManagerExt::GetInstance()->ActiveUserKey(user, token, secret), E_PARAMS_INVALID);
...
HWTEST_F(KeyManagerExtTest, KeyManagerExt_InActiveUserKey_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(false));
    EXPECT_EQ(KeyManagerExt::GetInstance()->InActiveUserKey(user), 0);
...
/**
 * @tc.name: KeyManagerExtTest_LockUserScreen_001
 * @tc.desc: Verify the KeyManagerExt LockUserScreen function.
 * @tc.type: FUNC
 * @tc.require: AR20250418146433
 */
HWTEST_F(KeyManagerExtTest, KeyManagerExtTest_LockUserScreen_001, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "KeyManagerExtTest_LockUserScreen_001 start";
    unsigned int user = 800;
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(false));
    EXPECT_EQ(KeyManagerExt::GetInstance()->InActiveUserKey(user), 0);
    ...
    GTEST_LOG_(INFO) << "KeyManagerExtTest_LockUserScreen_001 end";
}

/**
 * @tc.name: KeyManagerExt_UnlockUserScreen
 * @tc.desc: Verify the UnlockUserScreen function.
 * @tc.type: FUNC
 * @tc.require: AR20250418146433
 */
HWTEST_F(KeyManagerExtTest, KeyManagerExt_UnlockUserScreen, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "KeyManagerExt_UnlockUserScreen Start";
    unsigned int user = 888;
    const std::vector<uint8_t> token = {};
    const std::vector<uint8_t> secret = {};
    EXPECT_EQ(KeyManagerExt::GetInstance()->UnlockUserScreen(user, token, secret), E_OK);

    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(false));
    EXPECT_EQ(KeyManagerExt::GetInstance()->UnlockUserScreen(100, token, secret), E_OK);

    std::shared_ptr<BaseKey> tmpKey = std::dynamic_pointer_cast<BaseKey>(std::make_shared<FscryptKeyV2>("test"));
    KeyManager::GetInstance()->SaveUserElKey(user, EL1_KEY, tmpKey);
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(true));
    EXPECT_CALL(*baseKeyMock_, KeyDescIsEmpty()).WillOnce(Return(true));
    EXPECT_EQ(KeyManagerExt::GetInstance()->UnlockUserScreen(user, token, secret), E_PARAMS_INVALID);
    ...
    EXPECT_EQ(KeyManagerExt::GetInstance()->UnlockUserScreen(user, token, secret), E_OK);
    GTEST_LOG_(INFO) << "KeyManagerExt_UnlockUserScreen end";
}
...
HWTEST_F(KeyManagerExtTest, KeyManagerExt_SetRecoverKey_001, TestSize.Level1)
{
    ...
    std::vector<uint8_t> key;
     EXPECT_EQ(KeyManagerExt::GetInstance()->SetRecoverKey(key), E_OK);
 ...
 * @tc.require: AR20250418146433
 */
HWTEST_F(KeyManagerExtTest, KeyManagerExt_InactiveUserPublicDirKey_001, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "KeyManagerExt_InactiveUserPublicDirKey_001 Start";
    unsigned int user = 888;
    EXPECT_EQ(KeyManagerExt::GetInstance()->InactiveUserPublicDirKey(user), E_OK);
    ...
    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(true));
    EXPECT_CALL(*userkeyExtMocMock_, InactiveUserKey(_)).WillOnce(Return(E_PARAMS_INVALID));
    EXPECT_EQ(KeyManagerExt::GetInstance()->InactiveUserPublicDirKey(user), E_PARAMS_INVALID);

    EXPECT_CALL(*fscryptControlMock_, KeyCtrlHasFscryptSyspara()).WillOnce(Return(true));
    EXPECT_CALL(*userkeyExtMocMock_, InactiveUserKey(_)).WillOnce(Return(E_OK));
    EXPECT_EQ(KeyManagerExt::GetInstance()->InactiveUserPublicDirKey(user), E_OK);

    GTEST_LOG_(INFO) << "KeyManagerExt_InactiveUserPublicDirKey_001 end";
}
}
