...
/**
 * @tc.name: KeyMgrAnotherTest_EraseAllUserEncryptedKeys_001
 * @tc.desc: Verify KeyManager EraseAllUserEncryptedKeys when no OsAccounts returned.
 * @tc.type: FUNC
 * @tc.require:
 */
...
/**
 * @tc.name: KeyMgrAnotherTest_LockUserScreen_002
 * @tc.desc: Verify LockUserScreen returns E_USERID_RANGE with invalid userId.
 * @tc.type: FUNC
 * @tc.require: IAHHWW
 */
HWTEST_F(KeyMgrAnotherTest, KeyMgrAnotherTest_LockUserScreen_002, TestSize.Level1)
{
    uint32_t invalidUserId = static_cast<uint32_t>(StorageService::MAX_USER_ID) + 1;
    auto ret = KeyManager::GetInstance().LockUserScreen(invalidUserId);
    EXPECT_EQ(ret, E_USERID_RANGE);
    EXPECT_EQ(KeyManager::GetInstance().userPinProtect.count(invalidUserId), 0u);
    KeyManager::GetInstance().userPinProtect.clear();
    KeyManager::GetInstance().saveLockScreenStatus.clear();
}

/**
 * @tc.name: KeyMgrAnotherTest_UnlockUserScreen_002
 * @tc.desc: Verify UnlockUserScreen returns E_USERID_RANGE with invalid userId.
 * @tc.type: FUNC
 * @tc.require: IAHHWW
 */
HWTEST_F(KeyMgrAnotherTest, KeyMgrAnotherTest_UnlockUserScreen_002, TestSize.Level1)
{
    uint32_t invalidUserId = static_cast<uint32_t>(StorageService::MAX_USER_ID) + 1;
    std::vector<uint8_t> token;
    std::vector<uint8_t> secret;
    auto ret = KeyManager::GetInstance().UnlockUserScreen(invalidUserId, token, secret);
    EXPECT_EQ(ret, E_USERID_RANGE);
}

/**
 * @tc.name: KeyMgrAnotherTest_UpdateUserAuth_001
 * @tc.desc: Verify UpdateUserAuth returns E_USERID_RANGE with invalid userId.
 * @tc.type: FUNC
 * @tc.require: IAHHWW
 */
HWTEST_F(KeyMgrAnotherTest, KeyMgrAnotherTest_UpdateUserAuth_001, TestSize.Level1)
{
    uint32_t invalidUserId = static_cast<uint32_t>(StorageService::MAX_USER_ID) + 1;
    ...
    UserTokenSecret userTokenSecret = {token, oldSecret, newSecret, secureUid};
    auto ret = KeyManager::GetInstance().UpdateUserAuth(invalidUserId, userTokenSecret);
    EXPECT_EQ(ret, E_USERID_RANGE);
}

/**
 * @tc.name: KeyMgrAnotherTest_EraseAllUserEncryptedKeys_002
 * @tc.desc: Verify EraseAllUserEncryptedKeys skips out-of-range userIds.
 * @tc.type: FUNC
 * @tc.require: IAHHWW
 */
HWTEST_F(KeyMgrAnotherTest, KeyMgrAnotherTest_EraseAllUserEncryptedKeys_002, TestSize.Level1)
{
    ...
    auto ret = KeyManager::GetInstance().EraseAllUserEncryptedKeys(localIdList);
    EXPECT_EQ(ret, E_OK);
}
}
