...
/**
 * @tc.name: FileUtilsTest_IsFuse_002
 * @tc.desc: Verify the IsFuse function parameter handling and string comparison.
 * @tc.type: FUNC
 * @tc.require: AR000GK4HB
 */
HWTEST_F(FileUtilsTest, FileUtilsTest_IsFuse_002, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "FileUtilsTest_IsFuse_002 start";

    // Test the string comparison logic used in IsFuse
    const int FUSE_TRUE_LEN = 5;
    
    // Test various string comparison scenarios that IsFuse uses
    EXPECT_EQ(strncmp("true", "true", FUSE_TRUE_LEN), 0);
    EXPECT_NE(strncmp("false", "true", FUSE_TRUE_LEN), 0);
    EXPECT_NE(strncmp("TRUE", "true", FUSE_TRUE_LEN), 0); // Case sensitive
    EXPECT_NE(strncmp("", "true", FUSE_TRUE_LEN), 0);
    EXPECT_NE(strncmp("truee", "true", FUSE_TRUE_LEN), 0); // Length matters
    
    // Test the actual IsFuse function
    bool result = IsFuse();
    EXPECT_TRUE(result || !result);
    
    // Test parameter reading directly to understand the behavior
    char fuseEnterpriseEnable[7] = {"false"};
    int retEn = GetParameter("const.enterprise.external_storage_device.manage.enable", 
                           "", fuseEnterpriseEnable, 6);
    
    GTEST_LOG_(INFO) << "Parameter value: '" << fuseEnterpriseEnable 
                     << "', return code: " << retEn;
    
    // Test logic consistency: if parameter is "true", result should be true
    if (strncmp(fuseEnterpriseEnable, "true", 4) == 0) {
        EXPECT_TRUE(result);
    } else {
        EXPECT_FALSE(result);
    }

    GTEST_LOG_(INFO) << "FileUtilsTest_IsFuse_002 end";
}

/**
 * @tc.name: FileUtilsTest_IsFuse_003
 * @tc.desc: Verify the IsFuse function branch coverage.
 * @tc.type: FUNC
 * @tc.require: AR000GK4HB
 */
HWTEST_F(FileUtilsTest, FileUtilsTest_IsFuse_003, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "FileUtilsTest_IsFuse_003 start";

    // This test aims to cover both branches of the IsFuse function
    bool result = IsFuse();
    
    // Test constants used in IsFuse
    const int FUSE_VAL_LEN = 6;
    const int FUSE_TRUE_LEN = 5;
    const char* FUSE_PARAM_KEY = "const.enterprise.external_storage_device.manage.enable";
    
    // Test buffer initialization (like in IsFuse)
    char testBuffer[FUSE_VAL_LEN + 1] = {"false"};
    EXPECT_STREQ(testBuffer, "false");
    
    // Test parameter reading with the same parameters as IsFuse
    int retVal = GetParameter(FUSE_PARAM_KEY, "", testBuffer, FUSE_VAL_LEN);
    
    // Test the string comparison logic
    bool comparisonResult = (strncmp(testBuffer, "true", FUSE_TRUE_LEN) == 0);
    
    // The IsFuse result should match our manual comparison
    EXPECT_EQ(result, comparisonResult);
    
    // Log detailed information for debugging
    GTEST_LOG_(INFO) << "GetParameter return value: " << retVal;
    GTEST_LOG_(INFO) << "Buffer content: '" << testBuffer << "'";
    GTEST_LOG_(INFO) << "String comparison result: " << (comparisonResult ? "true" : "false");
    GTEST_LOG_(INFO) << "IsFuse result: " << (result ? "true" : "false");
    
    // Test that the function handles both possible branches
    if (comparisonResult) {
        EXPECT_TRUE(result); // Should return true when parameter is "true"
    } else {
        EXPECT_FALSE(result); // Should return false otherwise
    }
