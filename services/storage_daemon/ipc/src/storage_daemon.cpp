...
int32_t StorageDaemon::PrepareUserDirs(int32_t userId, uint32_t flags)
{
    ...
    if (ret != E_OK) {
        LOGE("GenerateUserKeys failed, userId %{public}u, flags %{public}u, error %{public}d", userId, flags, ret);
        return ret;
    }
#endif
    return prepareRet;
...
int32_t StorageDaemon::DestroyUserDirs(int32_t userId, uint32_t flags)
{
    ...
    destroyUserRet = KeyManagerExt::GetInstance()->DeleteUserKeys(userId);
    if (destroyUserRet != E_OK) {
        LOGE("DeleteUserKeys failed, userId %{public}u, flags %{public}u, error %{public}d", userId, flags, destroyUserRet);
        errCode = destroyUserRet;
    }
    return errCode;
#else
...
int32_t StorageDaemon::DeleteUserKeys(uint32_t userId)
{
    ...
    ret = KeyManagerExt::GetInstance()->DeleteUserKeys(userId);
    if (ret != E_OK) {
        LOGE("DeleteUserKeys failed, userId %{public}u, error %{public}d", userId, ret);
    }
    return ret;
#else
    return E_OK;
...
#ifdef USER_CRYPTO_MANAGER
    ret = KeyManagerExt::GetInstance()->ActiveUserKey(userId, token, secret);
    if (ret != E_OK) {
        LOGE("Active user key for userId=%{public}d failed, ret=%{public}d.", userId, ret);
        return ret;
    }
...
int32_t StorageDaemon::InactiveUserKey(uint32_t userId)
{
    ...
    if (ret != E_OK) {
        LOGE("InActive user key for userId=%{public}d failed, ret=%{public}d.", userId, ret);
    }
    return ret;
#else
    return E_OK;
...
int32_t StorageDaemon::LockUserScreen(uint32_t userId)
{
    ...
    ret = KeyManagerExt::GetInstance()->LockUserScreen(userId);
    if (ret != E_OK) {
        LOGE("Lock screen for userId=%{public}d failed, ret=%{public}d.", userId, ret);
    }
    return ret;
#else
    return E_OK;
                                        ...
                                        const std::vector<uint8_t> &secret)
{
    ...
    ret = KeyManagerExt::GetInstance()->UnlockUserScreen(userId, token, secret);
    if (ret != E_OK) {
        LOGE("UnlockUserScreen failed, userId=%{public}u, ret=%{public}d.", userId, ret);
    }
    return ret;
...
int32_t StorageDaemon::SetRecoverKey(const std::vector<uint8_t> &key)
{
    ...
    if (ret != E_OK) {
        LOGE("SetRecoverKey failed, please check");
    }
    ret = KeyManagerExt::GetInstance()->SetRecoverKey(key);
    if (ret != E_OK) {
        LOGE("SetRecoverKey failed, please check");
    }
    return ret;
...
int32_t StorageDaemon::InactiveUserPublicDirKey(uint32_t userId)
{
    int32_t ret = E_OK;
#ifdef USER_CRYPTO_MANAGER
    ret = KeyManagerExt::GetInstance()->InactiveUserPublicDirKey(userId);
    if (ret != E_OK) {
