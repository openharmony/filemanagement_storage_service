...
int32_t StorageDaemonProvider::DeletePartitionInfo(const std::string &devPath, const std::string &devName,
                                                     int32_t partitionNum)
{
    ...
    if (devName.empty()) {
        LOGE("[L1:StorageDaemonProvider] DeletePartitionInfo: <<< EXIT FAILED <<< devName empty");
        return E_PARAMS_INVALID;
    }
    if (partitionNum <= 0) {
...
#ifdef DISK_MANAGER
    LOGI("[L1:StorageDaemonProvider] DeletePartitionInfo: >>> ENTER <<< devPath=%{public}s, devName=%{public}s, partitionNum=%{public}d",
         devPath.c_str(), devName.c_str(), partitionNum);
    int32_t ret = DiskUtils::DeletePartitionInfo(devPath, devName, partitionNum);
    if (ret == E_OK) {
