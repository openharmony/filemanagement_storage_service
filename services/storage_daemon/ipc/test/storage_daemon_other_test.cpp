...
/**
 * @tc.name: StorageDaemonTest_SaveAndCleanKeyBuff_001
 * @tc.desc: Verify the SaveAndCleanKeyBuff when SaveStringToFileSync failed.
 * @tc.type: FUNC
 */
HWTEST_F(StorageDaemonTest, StorageDaemonTest_SaveAndCleanKeyBuff_001, TestSize.Level1)
{
    ASSERT_TRUE(storageDaemon_ != nullptr);
    ...
    EXPECT_CALL(*baseKeyMock_, SaveKeyBlob(_, _)).WillRepeatedly(Return(true));
    g_saveStringToFileSync = false;
...
/**
 * @tc.name: StorageDaemonTest_EncryptDe_001
 * @tc.desc: Verify the EncryptDe when SaveStringToFileSync succeed.
 * @tc.type: FUNC
 */
HWTEST_F(StorageDaemonTest, StorageDaemonTest_EncryptDe_001, TestSize.Level1)
{
    ASSERT_TRUE(storageDaemon_ != nullptr);
    std::shared_ptr<FscryptKeyV2> elKey = std::make_shared<FscryptKeyV2>("/data/test");
    UserAuth auth;
    std::string path = "/data/test";
    EXPECT_CALL(*huksMasterMock_, GenerateKey(_, _)).WillRepeatedly(Return(E_OK));
...
/**
 * @tc.name: StorageDaemonTest_EncryptDe_002
 * @tc.desc: Verify the EncryptDe when SaveStringToFileSync failed.
 * @tc.type: FUNC
 */
HWTEST_F(StorageDaemonTest, StorageDaemonTest_EncryptDe_002, TestSize.Level1)
{
    ASSERT_TRUE(storageDaemon_ != nullptr);
    std::shared_ptr<FscryptKeyV2> elKey = std::make_shared<FscryptKeyV2>("/data/test");
    UserAuth auth;
